...
function redirectWithError(
  ...
  return clearStateCookie(
    NextResponse.redirect(buildRedirectUrl(request, { gmail_error: code }))
  );
}

function redirectSuccess(request: NextRequest): NextResponse {
  return clearStateCookie(
    NextResponse.redirect(buildRedirectUrl(request, { gmail_connected: "1" }))
  );
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!tokens.refresh_token) {
      logError("gmail_oauth_error", new Error("no_refresh_token"), {
        step: "token_exchange",
        userId,
      });
