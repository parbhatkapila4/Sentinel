...
export function CustomerByCountry() {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex-1 relative aspect-2/1 min-h-[280px] w-full rounded-2xl overflow-hidden border border-white/10 bg-[#000000]">
        {isLoading ? (
          ...
          <>
            ...
            <p className="absolute bottom-2 left-2 text-[10px] text-white/50 pointer-events-none z-10">Drag to rotate</p>
          </>
        ) : (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center px-6">
              <p className="text-white/60 text-sm mb-2">No location data yet</p>
              <p className="text-white/50 text-xs">Add a country when creating a deal to see locations on the globe</p>
            </div>
          </div>
        )}
      </div>
      ...
      {!isLoading && top3Countries.length > 0 && (
        <div className="mt-4 pt-4 border-t border-white/10">
          <p className="text-[10px] font-medium text-white/60 uppercase tracking-wider mb-3">Top locations</p>
